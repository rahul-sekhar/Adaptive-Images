...
This fork optimises PNG images after scaling them down. It pngquant to be available as command line scripts. The latest compiled version is preferable, as it will result in better optimisation. By default PNGs are optimised to have 256 colours. To change this, add the number of colours before the extension. For example, some-image.16.png will have 16 colours. Make sure the originals are also optimised, as the originals are used if scaling is not required.
